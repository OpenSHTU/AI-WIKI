...
| 相似不变 | $\operatorname{tr}(P^{-1}AP) = \operatorname{tr}(A)$ |
...
| 性质 | 结论 |
|------|------|
| 上界 | $\operatorname{rank}(A) \le \min(m,n)$（$A \in \mathbb{R}^{m\times n}$） |
...
| 可逆乘 | $\operatorname{rank}(PAQ) = \operatorname{rank}(A)$（$P,Q$ 可逆） |
...
| 名称 | 定义 |
|------|------|
| $\ell_1$ 范数 | $\lVert\mathbf{x}\rVert_1 = \sum_i \lvert x_i\rvert$ |
| $\ell_2$ 范数（Euclidean） | $\lVert\mathbf{x}\rVert_2 = \sqrt{\sum_i x_i^2}$ |
| $\ell_p$ 范数 | $\lVert\mathbf{x}\rVert_p = \left(\sum_i \lvert x_i\rvert^p\right)^{1/p}$，$p \ge 1$ |
| $\ell_\infty$ 范数 | $\lVert\mathbf{x}\rVert_\infty = \max_i \lvert x_i\rvert$ |

### 11.2 矩阵范数

| 名称 | 定义 | 备注 |
|------|------|------|
| Frobenius 范数 | $\lVert A\rVert_F = \sqrt{\sum_{i,j}A_{ij}^2} = \sqrt{\operatorname{tr}(A^\top A)}$ | 与向量 $\ell_2$ 一致 |
| 谱范数（$\ell_2$ 算子范数） | $\lVert A\rVert_2 = \sigma_{\max}(A)$ | 最大奇异值 |
| 核范数 | $\lVert A\rVert_* = \sum_i \sigma_i$ | 奇异值之和 |
| $\ell_1$ 算子范数 | $\lVert A\rVert_1 = \max_j \sum_i \lvert A_{ij}\rvert$ | 最大列绝对和 |
| $\ell_\infty$ 算子范数 | $\lVert A\rVert_\infty = \max_i \sum_j \lvert A_{ij}\rvert$ | 最大行绝对和 |

一般的诱导矩阵范数定义为：

$$
\lVert A\rVert_p = \max_{\mathbf{x}\ne \mathbf{0}} \frac{\lVert A\mathbf{x}\rVert_p}{\lVert \mathbf{x}\rVert_p}
$$
...
| 条件 | 解的情况 |
|------|---------|
| $\operatorname{rank}(A) < \operatorname{rank}([A\,\vert\,\mathbf{b}])$ | 无解 |
| $\operatorname{rank}(A) = \operatorname{rank}([A\,\vert\,\mathbf{b}]) = n$ | 唯一解 |
| $\operatorname{rank}(A) = \operatorname{rank}([A\,\vert\,\mathbf{b}]) < n$ | 无穷多解 |
...
若 $A$ 列满秩，向量 $\mathbf{b}$ 在子空间 $\operatorname{col}(A)$ 上的正交投影：
